...
console.log(JSON.stringify({ task_id: result.task_id, status: result.status, evidence: result.evidence, verification: result.verification?.result, knowledge: result.knowledge, trace_events: result.trace.map((e) => e.event), control_events: controlPlane.events }, null, 2));
