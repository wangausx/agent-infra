...
const isolated = createIsolatedMissionControlServer({ port: Number(process.env.INTEGRATION_PORT ?? 0) });
...
try {
  await client.createTask({ title: 'isolated integration', description: 'Integration task\n\n### Acceptance Criteria\n- [ ] Verify native create returns a task\n- [ ] Check review output is in-review', status: 'backlog', assignee: 'main', priority: 'low', tags: ['agent-infra'], deliverables: [{ label: 'adapter documentation', path: 'README.md' }], task_contract: taskContract });
  const task = (await client.listTasks()).tasks[0];
  const claim = await client.claimTask(task.id, { owner: 'main' });
  await client.submitReview(task.id, { summary: 'The isolated adapter submitted a verifiable review result.', actor: 'main', claimId: claim.claim_id, claimGeneration: claim.claim_generation, deliverables: [{ label: 'adapter documentation', path: 'README.md' }] });
