...
export function planner({ taskId, objective, trace }) {
  ...
  return { plan, handoff: makeEnvelope({ taskId, runId: trace.runId, correlationId: trace.correlationId, sender: 'planner', recipient: 'executor', kind: 'plan', payload: plan }) };
}
...
export function executor({ taskId, plan, dryRun, approved, rollback, trace, faults = {} }) {
  ...
  return { evidence, handoff: makeEnvelope({ taskId, runId: trace.runId, correlationId: trace.correlationId, sender: 'executor', recipient: 'verifier', kind: 'evidence', payload: { evidence } }) };
}
...
export function verifier({ taskId, plan, evidence, trace, faults = {} }) {
  ...
  return { result, handoff: makeEnvelope({ taskId, runId: trace.runId, correlationId: trace.correlationId, sender: 'verifier', recipient: 'control-plane', kind: 'verification', payload: result }) };
}
